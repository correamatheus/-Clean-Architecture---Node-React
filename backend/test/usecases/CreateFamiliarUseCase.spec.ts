...
describe('CreateFamiliarUseCase', () => {
  it('Deve criar um familiar com dados válidos', async () => {
    ...
    const dto = { nome: 'Maria', dataNascimentoISO: '2000-02-02', identidade: '123456789', idPai: null };
    ...
    expect(created.identidade).toBe('123456789');
  ...
  it('Deve criar um familiar com referência ao pai existente', async () => {
    const repo = new InMemoryFamiliarRepository();
    const uc = new CreateFamiliarUseCase(repo);
    const pai = await uc.execute({ nome: 'João', dataNascimentoISO: '1970-01-01', identidade: '123456789', idPai: null });
    const dto = { nome: 'Ana', dataNascimentoISO: '2005-05-05', identidade: '123456789', idPai: pai.id };
  ...
  it('Deve lançar erro ao criar familiar sem nome', async () => {
    const repo = new InMemoryFamiliarRepository();
    const uc = new CreateFamiliarUseCase(repo);
    const dto = { nome: '', dataNascimentoISO: '2010-10-10', identidade: '123456789', idPai: null };
  ...
  it('Deve lançar erro ao criar familiar com data de nascimento inválida', async () => {
    const repo = new InMemoryFamiliarRepository();
    const uc = new CreateFamiliarUseCase(repo);
    const dto = { nome: 'Carlos', dataNascimentoISO: 'data-invalida', identidade: '123456789', idPai: null };
  ...
  it('Deve criar familiar sem informar idPai', async () => {
    const repo = new InMemoryFamiliarRepository();
    const uc = new CreateFamiliarUseCase(repo);
    const dto = { nome: 'Beatriz', identidade: '123456789', dataNascimentoISO: '1995-03-03' };
